...
int fork(void)
{
  ...
  acquire(&wait_lock);
  np->parent = p;
  release(&wait_lock); //For specification 1

  // copy saved user registers.
  *(np->trapframe) = *(p->trapframe);

  np->mask = p->mask; //For specification 1
...
int waitx(uint64 addr, uint *wait_time, uint *run_time)
{
  struct proc *np;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for (np = proc; np < &proc[NPROC]; np++)
    {
      if (np->parent == p)
      {
        ...
        if (np->state == ZOMBIE)
        {
          ...
          *wait_time = np->exit_time - np->run_time - np->c_time;
          if (addr != 0 && copyout(p->pagetable, addr, (char *)&np->xstate,
                                   sizeof(np->xstate)) < 0)
          {
            release(&np->lock);
